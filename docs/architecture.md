...
| New agent outbox command | Arm in `services/choruz-pipeline/src/outbox_handler.rs`, protocol text in `agent-templates` (bump `BOOTSTRAP_INSTRUCTION_VERSION` in `crates/choruz-host-runtime/src/instructions.rs` and add a fixture), tests beside the handler |
