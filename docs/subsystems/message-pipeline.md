...
| [`services/choruz-pipeline/src/pipeline.rs`](../../services/choruz-pipeline/src/pipeline.rs) | Spawns the task topology: CDC poller, router, dispatch, writer, lease monitor, retry scheduler, cron, outbox watcher, HTTP server |
...
| [`services/choruz-pipeline/src/pg_member_provider.rs`](../../services/choruz-pipeline/src/pg_member_provider.rs), [`pg_result_store.rs`](../../services/choruz-pipeline/src/pg_result_store.rs) | PostgreSQL adapters for the router (`PgMemberProvider`, `PgDecisionSink`), writer (`PgResultStore`) |
| [`crates/choruz-host-runtime/src/instructions.rs`](../../crates/choruz-host-runtime/src/instructions.rs), [`services/choruz-pipeline/src/rebootstrap.rs`](../../services/choruz-pipeline/src/rebootstrap.rs) | `ensure_claude_md`, `force_rewrite_bootstrap` (run by the pipeline for the gateway's device and by the connector for a remote one), `run_rebootstrap_command` (see [agent-protocol](agent-protocol.md)) |
...
| `BootstrapOutcome`, `PreservedEditedEntry` | [`crates/choruz-host-runtime/src/instructions.rs`](../../crates/choruz-host-runtime/src/instructions.rs) | Per-turn instruction refresh report |
...
`GET /v1/conversations/{conversation_id}/interactions` projects committed events
with their command and attempt outcomes for human operators with active
conversation access. `limit` is 1–100 (default 50); `before_seq` is an exclusive
cursor in descending sequence order. The response contains `records` and
`next_before_seq`. Content is omitted unless `include_content=true`; execution
prompts, raw errors and tool payloads are never included. Reads are audited as
`interaction.read`. The owner is
[`db_service/interactions.rs`](../../crates/choruz-application/src/db_service/interactions.rs),
not a second copy of the message store. Native terminal text is outside this
projection; [terminal activity](agent-runtime.md#entry-points) records transport
metadata instead.
...
| `CHORUZ_PIPELINE_METRICS_HOST`, `CHORUZ_PIPELINE_METRICS_PORT` | `127.0.0.1`, 3020 | Bind address of the health, readiness and metrics HTTP server |
...
4. `execute_command` calls `ExecutorContext::spawn_headless_session`: it resolves the agent's active `agent_runtime_bindings` row, requires `workspace_path` to exist, calls `ensure_claude_md(work_dir, driver_type)`, stages `metadata.attachments` into `<workspace>/.choruz-inbox/` (`choruz_host_runtime::inbox`, the connector does the same on a remote device), then spawns the CLI selected by `driver_type` with `HeadlessDriver::args` (Claude `--print --output-format stream-json`, Codex `exec --json`, Pi `--mode json`, Grok `-p --output-format streaming-json`, OpenCode `run --format json`) plus `CHORUZ_WORKSPACE`, `CHORUZ_SEND`, `CHORUZ_OUTBOX_DIR`, `DISABLE_AUTOUPDATER`, `PI_SKIP_VERSION_CHECK`, `CLAUDE_CODE_ENABLE_TASKS` and, for isolated harness accounts, the env returned by `harness_account_env`. `webhook_agent` bindings skip the spawn and only drain the outbox.
...
8. The product dashboard reads the gateway's `/v1/ws/sync` feed (see [sync-feed](sync-feed.md)).

Cron: `run_cron_scheduler` ticks every 30s, claims due `agent_cron_job` rows (`enabled`, `next_run_at <= NOW()`, `running_at` null or older than `CRON_STALE_CLAIM_AFTER` = 10 minutes) with `FOR UPDATE SKIP LOCKED`, inserts a `conversation_events` row from sender `choruz-cron` when `delivery_mode = 'announce'`, and inserts the `agent_commands` row directly. Jobs are created by the gateway routes `GET/POST /v1/agents/{agent_id}/cron` and `PATCH/DELETE /v1/agents/{agent_id}/cron/{job_id}` or by the `set_cron` outbox command. The command it inserts carries `metadata.runtime_host_id` through `choruz_session::runtime_host_metadata`, the same stamp `insert_command` applies, so a scheduled turn for an agent on a remote device is claimed by that device's connector rather than the local executor.

Outbox watcher: `run_outbox_watcher_loop` ticks every 2s over `RuntimeStore::list_active_bindings` (state not `disabled` or `paused`), drains only bindings that `should_drain_binding` selects (a `driver_type` passing `drains_via_watcher`, that is terminal drivers and `webhook_agent`, or any binding placed on a remote device, whose turns end there and whose commands only ever arrive in the mirror) and whose agent has no command in `leased`/`started`/`heartbeating`, and publishes any DM reply text with `publish_watcher_reply`. The directories it drains are `binding_outbox_dirs`: the workspace for a local binding; for a binding whose `config_json.runtime_host_id` names a remote device, every controller-side mirror `remote-outbox/<binding_id>/<host_id>` a device has shipped into (`remote_outbox_mirrors`; the connector fills the current device's through `POST /v1/runtime-hosts/{host_id}/bindings/{binding_id}/outbox`, and a shipment accepted in the moment the binding moved to another device is drained from the former device's mirror rather than lost).
...
HTTP on the metrics port: `GET /healthz` (`common::HostServiceStatus` for `choruz-pipeline`), `GET /readyz` (200 `ready` when both `EventStore::health_check` and `PgSessionStore::health_check` pass, else 503 `not_ready`), `GET /metrics` (`meta.rs`; the process-wide registry in `crates/choruz-common/src/metrics.rs` as Prometheus text, so a metric registered once with `common::metrics::register_counter` and its siblings from any crate the pipeline links appears without a handler change; `metrics_serves_the_shared_registry_as_prometheus_text`).

## Invariants

Scheduled instants use [`choruz_application::schedule::next_run_at`](../../crates/choruz-application/src/schedule.rs) for API creation, schedule or timezone edits, outbox creation, and recurring dispatch. Cron follows its IANA timezone (UTC when absent); intervals are elapsed durations; one-shot timestamps include an offset. Invalid values are rejected on creation or edit. Dispatch computes the next future occurrence without replaying missed calendar occurrences; the 30-second polling cadence still bounds normal dispatch latency.
...
- Unit tests inside [`dispatch.rs`](../../services/choruz-pipeline/src/dispatch.rs), [`executor.rs`](../../services/choruz-pipeline/src/executor.rs), [`outbox_handler.rs`](../../services/choruz-pipeline/src/outbox_handler.rs), [`outbox_watcher.rs`](../../services/choruz-pipeline/src/outbox_watcher.rs), [`cron_scheduler.rs`](../../services/choruz-pipeline/src/cron_scheduler.rs), [`instructions.rs`](../../crates/choruz-host-runtime/src/instructions.rs), [`config.rs`](../../services/choruz-pipeline/src/config.rs), [`meta.rs`](../../services/choruz-pipeline/src/meta.rs), [`pg_notify.rs`](../../services/choruz-pipeline/src/pg_notify.rs) and the `TestDatabase` cases in [`pg_member_provider.rs`](../../services/choruz-pipeline/src/pg_member_provider.rs); run with `cargo test -p choruz-pipeline`.
...
- Crate-local tests in [`crates/choruz-router/src/router.rs`](../../crates/choruz-router/src/router.rs), [`policy.rs`](../../crates/choruz-router/src/policy.rs), [`workflow.rs`](../../crates/choruz-router/src/workflow.rs), [`crates/choruz-writer/src/writer.rs`](../../crates/choruz-writer/src/writer.rs), [`crates/choruz-executor/src/wal.rs`](../../crates/choruz-executor/src/wal.rs), [`sandbox.rs`](../../crates/choruz-executor/src/sandbox.rs), [`crates/choruz-events/src/topics.rs`](../../crates/choruz-events/src/topics.rs), [`crates/choruz-store/src/cdc_poller.rs`](../../crates/choruz-store/src/cdc_poller.rs), [`event_outbox.rs`](../../crates/choruz-store/src/event_outbox.rs).
