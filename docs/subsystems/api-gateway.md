...
- [`plugins/mod.rs`](../../services/choruz-api-gateway/src/plugins/mod.rs): `registrations()` lists the built-in Host plugins; a plugin's router is merged only when `common::plugins::plugin_enabled(id)` is true for the `CHORUZ_PLUGINS` allowlist. Routers exist for `kanban`, `remote-ssh`, and `remote-control`; the remaining plugins advertise capabilities through manifests. Pi and OpenCode scan/import requests also require their plugins. See [plugin configuration](../plugins.md).
...
- Browser workflow admission in [`handlers_browser_automation.rs`](../../services/choruz-api-gateway/src/handlers_browser_automation.rs) uses human-configured standing scope. The bound Agent requests current tasks; server-owned learning revisions and binding configuration determine execution. Durable receipts and completion commands survive gateway restart. The device owns browser and native assistance under one deadline. See the [decision package](../../crates/choruz-decision/README.md) for permission, validation and cancellation semantics.
