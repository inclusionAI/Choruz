...
Group provisioning sets the policy: `defaultEnableRoutingPolicy` in `apps/web/lib/groups/group-provisioning-runner.ts` writes `default_coordinator_agent_id` and `untagged_human_mode: "coordinator_only"` through `PUT /v1/runtime/policies/{conversation_id}` only when the template maps an explicit coordinator slot (`coordinatorRoleSlotId`) to an agent; display names are never inspected. `RuntimeStore::upsert_policy` (`crates/choruz-application/src/runtime_store/policy.rs`) rejects a coordinator that is not a principal, and the column carries `ON DELETE SET NULL`.
...
- Policy persistence: `policy_defaults_and_upsert_work` and `policy_rejects_invalid_default_coordinator` in `crates/choruz-application/tests/runtime_store.rs`.
