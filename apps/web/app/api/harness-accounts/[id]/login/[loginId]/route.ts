...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string; loginId: string }> }) {
  ...
  const response = await fetch(
    `${apiBaseUrl()}/v1/companies/${encodeURIComponent(companyId)}/harness-accounts/${encodeURIComponent(account.id)}/logins/${encodeURIComponent(loginId)}`,
    { headers: { authorization: `Bearer ${auth.token}` }, cache: "no-store" },
  );
  const body = await response.json().catch(() => ({ error: "Unable to read the sign-in" }));
