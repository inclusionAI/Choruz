...
- A delegation does not need to repeat a new card's `task_key` in chat: the assignee must use the matching authoritative card from its incoming `your_tasks:` field. When a coordinator must verify or summarize the result, the owner must submit the card update and an `@mention` completion report in the same turn, with the task command first. Command-result files are emitted after the turn, so never tell the owner to wait for one before sending that report. If the coordinator later asks the owner to inspect that file, the verification reply must also `@mention` the coordinator. A concise-result request such as "report only the number" limits the narrative payload; it never removes the task mutation or routing mention.
