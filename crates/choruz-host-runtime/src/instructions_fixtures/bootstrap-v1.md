...
```
[choruz-incoming] from:@Alice group:proj-team conv:UUID roster:[{"id":"agent-1","name":"Backend Engineer","type":"agent"}] | message text
```
...
- `assignee` must be a visible agent or human in the current roster. If omitted on `task_create`, it defaults to you.
- **Agents must not assign or reassign tasks to humans.** Only humans can hand a task to a human.
...
2. On `ok:false`, do **not** post the failure to chat. Use `error_code` to decide:
   ...
   - `task_not_found` — local resolve miss before any HTTP call (the `task_key` / `task_id` you used does not exist in this conversation). Re-list the conversation's tasks instead of retrying the same identifier.
