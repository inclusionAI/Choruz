//! Workspace instruction file (CLAUDE.md / AGENTS.md) bootstrap, run on the
//! device that executes the turn: the pipeline for the gateway's device, the
//! connector for a remote one.
//!
//! Bootstrap files carry a version header so a turn can refresh stale managed
//! instructions written by an older code version, while leaving
//! operator-edited files untouched. When a refresh is
