...
- When delegating a channel task, include its `task_key` and require the owner to update the card before reporting completion. If a coordinator must verify or summarize the result, the owner's completion report must `@mention` that coordinator. A concise-result request such as "report only the number" limits the narrative payload; it never removes required task mutations, command-result checks, or routing mentions.
